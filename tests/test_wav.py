"""
Unit tests for app._write_debug_wav().
...
from app import _write_debug_wav
...
def test_writes_file(tmp_wav):
    _write_debug_wav(_pcm_bytes(), tmp_wav)
...
def test_wav_frame_count_matches_input(tmp_wav):
    n = 16000
    _write_debug_wav(_pcm_bytes(n), tmp_wav)
    with wave.open(str(tmp_wav), "rb") as wf:
